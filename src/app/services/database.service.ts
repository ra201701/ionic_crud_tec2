...
export class DatabaseService {
  ...
  constructor(private plt: Platform, private sqlitePorter:
    SQLitePorter, private sqlite: SQLite, private http: HttpClient) {
  ...
  loadStudents() {
    return this.database.executeSql('SELECT * FROM Students',
      []).then(data => {
  ...
  updateStudent(student: Student) {
    ...
    return this.database.executeSql(`UPDATE Students SET name = ?,
          class = ?, mark = ? WHERE studId = ${student.studId}`, data).then(data => {
      this.loadStudents();
    });
  }
